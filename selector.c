...
void selector_init(struct selector_t *sel, struct listing_t *lst)
{
    ...
    listing_match(sel->base_listing, sel->view_listing, sel->search_string);
}
...
{
    if(sel->search_len > 0)
        sel->search_string[--sel->search_len] = '\0';
...
void selector_search_refine(struct selector_t *sel, char key)
{
    ...
    sel->search_string[sel->search_len] = key;
    sel->search_string[++sel->search_len] = '\0';

    listing_blank(sel->swap_listing);
    listing_match(sel->view_listing, sel->swap_listing, sel->search_string);
